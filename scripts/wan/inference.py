...
from vorta.patch.modeling_wan import (
    apply_sp_flashattn_transformer,
    apply_vorta_transformer,
)
from vorta.patch.pipeline_wan import sp_pipeline_call, vorta_pipeline_call
...
BENCHMARK_VIDEO_SIZE = (77, 720, 1280)  # (num_frames, height, width)
...
def parse_args() -> Namespace:
    ...
    parser.add_argument("--output_fps", type=int, default=15)
    ...
    parser.add_argument(
        "--cfg_scale", type=float, default=5.0,
        help="The embedded guidance scale as a condition for the model. The default value is 6.0."
    )
    parser.add_argument("--video_size", type=int, nargs=3, default=[77, 720, 1280])
    ...
    if tuple(args.video_size) != BENCHMARK_VIDEO_SIZE:
        raise ValueError(f"Only supports {BENCHMARK_VIDEO_SIZE} video size, got {args.video_size}")
...
def load_model(args: Namespace, device: torch.device):
    ...
    with open(f"{args.pretrained_model_path}/scheduler/scheduler_config.json", "r") as f:
        scheduler_config = json.load(f)
    scheduler_config.update({"flow_shift": args.noise_scheduler_shift})
