...
from scripts.wan.train_one_step import train_one_step

from vorta.constants import STA_TILE_SIZE
from vorta.dataset import LatentDataset, sp_parallel_dataloader_wrapper
from vorta.patch.modeling_wan import apply_vorta_transformer
from vorta.patch.utils import prepare_wan_self_attn_kwargs, wan_pixel2token
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train Wan model FSDP")
    parser.add_argument("--exp_dir", type=Path, default=Path("/raid/data/runs/vorta/wan-14B/debug"))
    parser.add_argument("--seed", type=int, default=None, help="Random seed")

    # data
    parser.add_argument(
        '--data_json_file', type=Path, 
        default=Path('/raid/data/datasets/HD-Mixkit-resized-2k/Finetune-Wan_2_1-720p/videos2caption.json'))
    ...
    parser.add_argument('--video_size', type=int, nargs=3, default=[77, 720, 1280], help='Video size')

    # model
    parser.add_argument('--pretrained_model_path', type=str, default="/raid/data/models/Wan2.1-T2V-14B-Diffusers")
    ...
    parser.add_argument(
        '--lowres_window_size', type=int, nargs=3, default=[2, 3, 2], help='Low resolution window size')
    ...
    parser.add_argument(
        '--sliding_attn_tile_size', type=int, nargs=3, default=[5, 9, 8], help='Sliding attention tile size')
    ...
    parser.add_argument('--reg_loss_weight', type=float, default=0.1, help='Regularization loss weight')
    parser.add_argument(
    ...
    args.latent_shape = wan_pixel2token(args.video_size)
...
def main():
    ...
    train_dataset = LatentDataset(
        args.data_json_file, args.latent_shape[0], args.data_uncond_rate, model_name='wan', device=device)
    ...
    ).to(dtype=torch.bfloat16)  # explicitly set LayerNorm to bfloat16
    ...
    self_attention_kwargs = prepare_wan_self_attn_kwargs(self_attention_kwargs, device=device)
    ...
    transformer = wrap_wan_with_fsdp(
