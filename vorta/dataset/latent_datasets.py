...
# -*- coding: utf-8 -*-
'''
@Created :   2025/03/12 18:53:20
@Desc    :   
    2025/03/13: add pooled_prompt_embeds; it has been included in `prompt_embed[0, :768]`
    2025/03/14: attention_mask is not used in the model, remove it
@Ref     :   Copy from https://github.com/hao-ai-lab/FastVideo/blob/main/fastvideo/dataset/latent_datasets.py
'''
import json
import os
import random
...
class LatentDataset(Dataset):
    def __init__(
        ...
        # just zero embeddings [256, 4096] with # 256 zeros mask
        self.uncond_prompt_embed = torch.zeros(seq_len, hidden_dim).to(device, torch.float32)
        if have_mask:
            self.uncond_prompt_mask = torch.zeros(seq_len).bool().to(device)
        else:
            self.uncond_prompt_mask = None

        # for dynamic batching (not implemented yet)
    ...
    def __getitem__(self, idx):
        ...
        if random.random() < self.cfg_rate:
            ...
            if self.uncond_prompt_mask is None:
        ...
        prompt_embed.requires_grad_(False)
    ...
    def collate_function(batch):
        ...
        if prompt_attention_masks[0] is not None:
...
if __name__ == "__main__":
    ...
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=2, shuffle=False, collate_fn=dataset.collate_function)
